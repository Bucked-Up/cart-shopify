...
const main = async () => {
  toggleLoading();
  const [data, orderBumpData] = await Promise.all([fetchProduct({ ids: productsID }), fetchProduct({ ids: "multiBump" in orderBumpIds ? Object.keys(orderBumpIds.multiBump.products) : Object.keys(orderBumpIds), isOrderBump: true })]);
  dataLayerStart(data);
